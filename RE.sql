...
FROM (
									...
									CASE
										WHEN (START_DATE IS NOT NULL AND COMPLT_DATE IS NOT NULL) 
											THEN --装修已完成
												(
													CASE
														WHEN (START_DATE<:FROM_DATE AND COMPLT_DATE<:TO_DATE)   THEN (COMPLT_DATE-:FROM_DATE)+1     --1 c-f
														WHEN (START_DATE>:FROM_DATE  AND COMPLT_DATE>:TO_DATE)  THEN (:TO_DATE-START_DATE)+1        --2 t-s
														WHEN (START_DATE>=:FROM_DATE  AND COMPLT_DATE<=:TO_DATE)   THEN (COMPLT_DATE-START_DATE)+1  --3 c-s
														WHEN (START_DATE<:FROM_DATE AND COMPLT_DATE>:TO_DATE)  THEN (:TO_DATE-:FROM_DATE)+1         --4 t-f
														ELSE 0
													END
												)
    ...
    FROM (
		...
		--LEFT JOIN BG_DTL BD ON B.BRAND_ID=BD.BRAND_ID
		--LEFT JOIN BG BG ON BG.BG_ID=BD.BG_ID
	...
	WHERE 1=1
	AND (LENGTH (D.UNIT_CODE)=6 OR LENGTH (D.UNIT_CODE)='10')
	AND (
			U.UNIT_CODE LIKE '8%' --加盟
			OR (U.UNIT_CODE LIKE '9%' AND U.LIC_TYPE='D301' AND U.UNIT_NAME NOT LIKE '%公司%' AND U.UNIT_NAME NOT LIKE '%仓库%')  --直营
			OR  U.UNIT_CODE LIKE '7%' --联营
			AND D.UNIT_STATUS ='A'
		)
